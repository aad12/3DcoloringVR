...
public class ColorSelector : MonoBehaviour
{
	...
	void UserInputUpdate()
    {
		...
		if(Physics.Raycast(Rhand.transform.position, Rhand.transform.forward, out hit, Mathf.Infinity)){ //Physics.Raycast(cursorRay,out hit)){
           
            Vector3 localPosition=transform.InverseTransformPoint(hit.point);

			float dist=Vector2.Distance(Vector2.zero,localPosition);
            
            if (dist > 0.22 && hit.transform.tag == "palette")
            {
                SelectOuterColor(localPosition);
            }
            else if (dist <= 0.22 && hit.transform.tag == "palette")
            {
